...
- disabled turn 的 response/state/snapshot 不得泄漏 retrieval citations、external sources、related assets 或生成正文。
- 若本轮显式指定了用户可读的 target document，finalizer 可以保留该 document scope；不得把它误当 source leakage 清除。下一轮补充具体步骤、参数或检查点时，必须在同一 target document 上恢复 grounded detail flow。
- capability 重新启用时，正常路由可以恢复。
...
- composite deterministic router 中的其他 builder 可能先于 route-gated dispatch 命中；pre-split kill switch 必须直接消费 `try_build_route_gated_document_answer`，并测试其他 builders 未调用。
- route 被选中但检索结果为空。
